...
class EpisodeEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = TheOfficeSDK::test($seed, null);
        $seen = iterator_to_array($base->Episode(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = TheOfficeConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = TheOfficeSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set THE_OFFICE_TEST_EPISODE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function episode_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("THE_OFFICE_TEST_EPISODE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["THE_OFFICE_TEST_EPISODE_ENTID"]);
