...
class FtCtrl extends TheOfficeControl
{
...
class FtClient
{
    ...
    private TheOfficeUtility $utility;

    public function __construct(TheOfficeUtility $utility, array $options)
    {
    ...
    public function get_utility(): TheOfficeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TheOfficeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TheOfficeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new TheOfficeUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (TheOfficeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TheOfficeConfig::make_feature($name);
    ...
    private static function buildUrl(TheOfficeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TheOfficeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TheOfficeResponse($fetched);
        }
    ...
    private function fail(TheOfficeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TheOfficeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
