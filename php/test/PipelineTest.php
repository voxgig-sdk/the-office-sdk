...
{
    private static function utility(): TheOfficeUtility
    {
        return new TheOfficeUtility();
    }
    ...
    private static function ctx(array $over = []): TheOfficeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TheOfficeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TheOfficeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TheOfficeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TheOfficeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TheOfficeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TheOfficeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TheOfficeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TheOfficeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TheOfficeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TheOfficeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TheOfficeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TheOfficeResponse([]),
            'result' => new TheOfficeResult([]),
        ]);
        [, $err] = TheOfficeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TheOfficeSpec([]),
            'response' => null,
            'result' => new TheOfficeResult([]),
        ]);
        [, $err] = TheOfficeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TheOfficeSpec(['step' => 's']),
            'response' => new TheOfficeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TheOfficeResult([]),
        ]);
        [, $err] = TheOfficeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TheOfficeSpec(['step' => 's']),
            'response' => new TheOfficeResponse(self::resp(200, ['v' => 1])),
            'result' => new TheOfficeResult([]),
        ]);
        [, $err] = TheOfficeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TheOfficeSpec(['step' => 's']),
            'response' => new TheOfficeResponse(self::resp(200, ['v' => 2])),
            'result' => new TheOfficeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TheOfficeMakeResponse::call($ctx);
    ...
    {
        $preset = new TheOfficeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TheOfficeSpec([]),
            'response' => new TheOfficeResponse([]),
            'result' => new TheOfficeResult([]),
        ...
        [$got, $err] = TheOfficeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TheOfficeResult([])]);
        [, $err] = TheOfficeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TheOfficeSpec([]), 'result' => null]);
        [, $err] = TheOfficeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TheOfficeSpec(['step' => 's']),
            'result' => new TheOfficeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TheOfficeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TheOfficeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TheOfficeSpec(['step' => 's']),
            'result' => new TheOfficeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TheOfficeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TheOfficeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TheOfficeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TheOfficeSpec([]),
            'result' => new TheOfficeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TheOfficeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TheOfficeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TheOfficeError('boom', 'boom');
        $utility->fetcher = function (TheOfficeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TheOfficeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TheOfficeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TheOfficeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TheOfficeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TheOfficeResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TheOfficeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TheOfficeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TheOfficeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TheOfficeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TheOfficeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TheOfficeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TheOfficeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TheOfficeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TheOfficeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TheOfficeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TheOfficeResult(['ok' => false])]);
        $this->expectException(TheOfficeError::class);
        TheOfficeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TheOfficeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TheOfficeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TheOfficeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TheOfficeBaseFeature();
        $b = new TheOfficeBaseFeature();
        TheOfficeFeatureAdd::call($ctx, $a);
        TheOfficeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TheOfficeBaseFeature
    {
        $f = new TheOfficeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TheOfficeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TheOfficeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TheOfficeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TheOfficePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TheOfficeResponse(['headers' => null]),
            'result' => new TheOfficeResult([]),
        ]);
        TheOfficeResultHeaders::call($ctx);
