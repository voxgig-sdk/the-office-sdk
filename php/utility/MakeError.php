...
class TheOfficeMakeError
{
    public static function call(?TheOfficeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TheOfficeContext([], null);
        }
        $op = $ctx->op ?? new TheOfficeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TheOfficeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TheOfficeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TheOfficeError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof TheOfficeError) {
