...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.THEOFFICE_TEST_LIVE ||
    'TRUE' === process.env.THEOFFICE_TEST_OVERRIDE
  ) {
  ...
  m.THEOFFICE_TEST_EXPLAIN = process.env.THEOFFICE_TEST_EXPLAIN || m.THEOFFICE_TEST_EXPLAIN
