...
//     (\`new TheOfficeSDK(...)\` / \`TheOfficeSDK.test(...)\` become
//     \`TheOfficeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"episode":{"test01":{"id":"test01"}},"season":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TheOfficeSDK(...)\` and any
// \`TheOfficeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TheOfficeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TheOfficeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
