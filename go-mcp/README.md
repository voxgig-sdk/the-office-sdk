...
[MCP](https://modelcontextprotocol.io) server exposing the TheOffice SDK as
two agent tools — `the-office_list` and `the-office_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/the-office-mcp)
make build

# 2. Provide credentials via the environment
export THE_OFFICE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user the-office \
  -- /absolute/path/to/the-office-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./the-office-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// the-office_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// the-office_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/the-office-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user the-office \
     -- "$PWD"/dist/*/the-office-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `the-office_list` and `the-office_load` tools now appear
   in new sessions. Ask the agent to *"list character using the-office"*
   and it calls `the-office_list` with `{"entity":"character"}`.
...
```sh
export THE_OFFICE_APIKEY=sk_live_xxx            # API key
export THE_OFFICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `the-office_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `the-office_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `THE_OFFICE_APIKEY` | API key sent with every request. |
| `THE_OFFICE_BASE` | Optional override of the API base URL. |
...
```sh
./the-office-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"the-office_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
