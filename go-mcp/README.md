...
MCP server exposing the TheOffice SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `the-office_list` | `entity`, optional `query` map | First page of records as JSON |
| `the-office_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `the-office_list` and
`the-office_load` tools become available in new sessions.
