...
boru-driven command-line client **and** interactive REPL for the TheOffice
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/the-office-cli)
make build
...
./the-office-cli list character
./the-office-cli load 1 character            # {id:1} shorthand
./the-office-cli load '{id:1}' character       # explicit match map
./the-office-cli list episode

# 5. Override the API base URL for a single call
THE_OFFICE_BASE=https://api.example.com ./the-office-cli list character

# 6. No arguments -> interactive REPL
./the-office-cli
the-office> list character
the-office> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/the-office-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/the-office-cli list character
   ```
...
```sh
./the-office-cli load 1 character          # scalar shorthand for {id:1}
./the-office-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `the-office>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/the-office-cli`. |
...
(`./the-office-cli <expr>`) and interactively (the REPL), and expressions compose the
