// Command TheOffice-cli is a CLI + REPL for the TheOffice SDK,
...
const prompt = "the-office"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "character episode season"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      THE_OFFICE_APIKEY (api key), THE_OFFICE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: THE_OFFICE_APIKEY carries the API key and
	// THE_OFFICE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("THE_OFFICE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("THE_OFFICE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTheOfficeSDK(opts)
