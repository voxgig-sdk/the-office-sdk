...
class EpisodeEntity
  ...
  def data_set(args)
    if args
      @_data = TheOfficeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TheOfficeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Episode>, Array] the matching Episode items; raises TheOfficeError on failure
