...
require_relative 'TheOffice_types'
...
class TheOfficeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TheOfficeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TheOfficeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TheOfficeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TheOfficeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TheOfficeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TheOfficeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TheOfficeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = TheOfficeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TheOfficeError.new(
        "graphql_error", "TheOfficeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
