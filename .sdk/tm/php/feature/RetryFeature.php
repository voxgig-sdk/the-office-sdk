...
class TheOfficeRetryFeature extends TheOfficeBaseFeature
{
    ...
    public function init(TheOfficeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TheOfficeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TheOfficeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TheOfficeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
