...
class TheOfficeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TheOfficeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TheOfficeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TheOfficeTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = TheOfficeTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TheOfficeTestRunner.getenv(key)
        ...
        explain = TheOfficeTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TheOfficeTestRunner._test_control is not None:
            return TheOfficeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TheOfficeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TheOfficeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TheOfficeTestRunner.load_test_control()
...
def load_env_local():
    TheOfficeTestRunner.load_env_local()


def env_override(m):
    return TheOfficeTestRunner.env_override(m)


def entity_data(v):
    return TheOfficeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TheOfficeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TheOfficeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TheOfficeTestRunner.load_test_control()


def live_client_options():
    return TheOfficeTestRunner.live_client_options()


def live_delay_ms():
    return TheOfficeTestRunner.live_delay_ms()
