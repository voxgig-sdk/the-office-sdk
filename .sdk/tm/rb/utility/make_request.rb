...
module TheOfficeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TheOfficeResponse.new({})
    ...
    if fetch_err
      ...
      response = TheOfficeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TheOfficeResponse.new(fetched)
    else
