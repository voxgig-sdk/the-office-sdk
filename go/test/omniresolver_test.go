...
import (
	...
	omni "github.com/voxgig-sdk/the-office-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TheOfficeError); is {
				out := map[string]any{
					"name":    "TheOfficeError",
...
func omniCtx(arg any, client *sdk.TheOfficeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TheOfficeSDK); is {
			client = live
